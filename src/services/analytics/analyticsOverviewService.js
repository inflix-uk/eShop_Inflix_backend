...
async function getAnalyticsOverview(query = {}) {
  ...
  const [facetResult] = await Order.aggregate([
    { $facet: {
      ...
      revenueBySource: [
        ...
        {
          $match: {
            'marketingAttribution.normalized.source': {
              $exists: true,
              $nin: [null, ''],
            },
          },
        ...
        {
          $group: {
            _id: '$marketingAttribution.normalized.source',
            revenue: { $sum: { $ifNull: ['$totalOrderValue', 0] } },
            orders: { $sum: 1 },
          },
        },
      ...
      revenueByMedium: [
        ...
        {
          $match: {
            'marketingAttribution.normalized.medium': {
              $exists: true,
              $nin: [null, ''],
            },
          },
        ...
        {
          $group: {
            _id: '$marketingAttribution.normalized.medium',
            revenue: { $sum: { $ifNull: ['$totalOrderValue', 0] } },
            orders: { $sum: 1 },
          },
        },
      ...
      revenueByCampaign: [
        ...
        {
          $match: {
            'marketingAttribution.normalized.campaign': {
              $exists: true,
              $nin: [null, ''],
            },
          },
  ...
  const revenueBySource = (facetResult.revenueBySource || []).map((row) => ({
    source: row._id,
    revenue: round2(row.revenue),
    orders: row.orders,
  }));

  const revenueByMedium = (facetResult.revenueByMedium || []).map((row) => ({
    medium: row._id,
    revenue: round2(row.revenue),
    orders: row.orders,
  }));

  const revenueByCampaign = (facetResult.revenueByCampaign || []).map((row) => ({
    campaign: row._id,
    revenue: round2(row.revenue),
    orders: row.orders,
  }));
